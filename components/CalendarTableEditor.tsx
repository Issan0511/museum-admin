      ...
      <div className="rounded border border-slate-200 bg-white p-4 shadow-sm">
        <div className="flex flex-wrap items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              type="button"
              className="rounded border border-emerald-300 px-2 py-1 text-sm font-medium text-emerald-700 hover:bg-emerald-50"
              onClick={goToToday}
            >
              今日
            </button>
      ...
      <div className="grid gap-6 lg:grid-cols-[1.2fr,1fr]">
        <div className="rounded border border-slate-200 bg-white p-4 shadow-sm">
          <div className="flex items-center justify-between">
            ...
            <button
              type="button"
              className="rounded bg-indigo-600 px-3 py-1 text-sm font-medium text-white hover:bg-indigo-700 disabled:bg-indigo-300"
              onClick={() => startCreate()}
              disabled={isSubmitting || templates.length === 0}
            >
              新規追加
          ...
          <div className="mt-4 overflow-x-auto">
            <table className="min-w-full divide-y divide-slate-200 text-sm">
              <thead className="bg-slate-100">
                <tr>
                  <th className="px-3 py-2 text-left font-medium uppercase tracking-wide text-slate-600">{primaryKey}</th>
                  <th className="px-3 py-2 text-left font-medium uppercase tracking-wide text-slate-600">日付</th>
                  <th className="px-3 py-2 text-left font-medium uppercase tracking-wide text-slate-600">実演</th>
                  <th className="px-3 py-2 text-left font-medium uppercase tracking-wide text-slate-600">操作</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-200">
                {sortedRows.map((row) => {
                  const pkValue = row[primaryKey];
                  const rowKey = pkValue !== undefined && pkValue !== null ? String(pkValue) : JSON.stringify(row);
                  const templateId = normalizeTemplateId(row.template_id);
                  const templateName = templateId !== null ? templateMap.get(templateId)?.name_ja : undefined;
                  const isEditing = editingKey !== null && rowKey === editingKey;
                  return (
                    <tr key={rowKey} className={isEditing ? "bg-emerald-50" : "odd:bg-slate-50"}>
                      <td className="px-3 py-2 text-slate-600">{pkValue ?? "-"}</td>
                      <td className="px-3 py-2 text-slate-700">{formatDisplayDate(row.demo_date ?? null) || "-"}</td>
                      <td className="px-3 py-2 text-slate-700">
                        {templateName || (templateId !== null ? `ID: ${templateId}` : "未設定")}
                      </td>
                ...
                {sortedRows.length === 0 && (
                  <tr>
                    <td colSpan={4} className="px-3 py-6 text-center text-sm text-slate-500">
                      予定が登録されていません。カレンダーまたは「新規追加」から登録してください。
                    </td>
                  </tr>
                )}
              </tbody>
          ...
          <div className="mt-4 space-y-4">
            <div>
              ...
              <input
                ...
                disabled={isSubmitting || formState.mode === "idle"}
              />
            </div>
            <div>
              <label className="text-sm font-medium text-slate-700">実演</label>
              <select
                className="mt-1 w-full rounded border border-slate-300 px-3 py-2 text-sm"
                value={formState.values.template_id}
                onChange={(event) => handleChange("template_id", event.target.value)}
                disabled={isSubmitting || formState.mode === "idle" || templates.length === 0}
              >
                {templates.length === 0 ? (
            ...
            <div className="flex gap-2">
              {formState.mode !== "idle" ? (
                <>
                  <button
                    type="button"
                    className="rounded bg-emerald-600 px-3 py-2 text-sm font-medium text-white hover:bg-emerald-700 disabled:bg-emerald-300"
                    onClick={submitForm}
                    disabled={isSubmitting}
                  >
                  ...
                  <button
                    type="button"
                    className="rounded border border-slate-300 px-3 py-2 text-sm font-medium text-slate-600 hover:bg-slate-100 disabled:opacity-50"
                    onClick={resetForm}
                    disabled={isSubmitting}
                  >
                    キャンセル
                ...
                <button
                  type="button"
                  className="rounded bg-indigo-600 px-3 py-2 text-sm font-medium text-white hover:bg-indigo-700 disabled:bg-indigo-300"
                  onClick={() => startCreate()}
                  disabled={isSubmitting || templates.length === 0}
                >
                  予定を追加
